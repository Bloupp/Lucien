...
cfgusuelle = [planetes,m,a,e,lpr,th,incl]
...
def TabSolGeneral(l,masse,dga,ex,wb,teta,phi):
    n = len(ex)
    H01 = [ex[i]*np.sin(wb[i]) for i in range(n)]
    L01 = [ex[i]*np.cos(wb[i]) for i in range(n)]
    P01 = [np.tan(phi[i])*np.sin(teta[i]) for i in range(n)]
    Q01 = [np.tan(phi[i])*np.cos(teta[i]) for i in range(n)]
    H1,L1 = solution(l,masse,dga,H01,L01)
    P1,Q1 = solution(l,masse,dga,P01,Q01)
    return [H1,L1,P1,Q1]
...
#Pour les fonctions suivantes il faut gérer les cas ou tan passe par pi/2 à la main
...
plt.figure()
R = TabSolGeneral(planetes,m,a,e,lpr,th,incl)
...
plt.figure()
court = 20000
Tcourt = np.linspace(-court*annee,court*annee,20000)
Tcourt_aff = np.linspace(-court,court,20000)
for i in range(2,3):
    N = [theta(i,t,R)*180/pi for t in Tcourt]
    plt.plot(Tcourt_aff,N, lw=2)
plt.grid()
plt.show()
...
def aff1ExcGeneral(l,masse,dga,ex,wb,teta,phi,t1,t2,n,L):
    R1 = TabSolGeneral(l,masse,dga,ex,wb,teta,phi)
...
def affiche2Exc(R1,R2,t1,t2,n,L):
    ...
    for i in L:
        ...
        plt.plot(Taff,E1, lw=2)
        E2 = [excentricite(i,t,R2) for t in T]
        plt.plot(Taff,E2, lw=2)
    plt.ylabel("Excentricités")
    plt.xlabel("Temps - à 0 à l'instant présent (en années)")
    plt.title("Variation des excentricités au cours du temps")
    plt.grid()
...
def aff2ExcGeneral(cfg1,cfg2,t1,t2,n,L):
    [l1,m1,a1,e1,wb1,teta1,phi1] = cfg1
    [l2,m2,a2,e2,wb2,teta2,phi2] = cfg2
    R1 = TabSolGeneral(l1,m1,a1,e1,wb1,teta1,phi1)
    R2 = TabSolGeneral(l2,m2,a2,e2,wb2,teta2,phi2)
    affiche2Exc(R1,R2,t1,t2,n,L)

R1 = TabSolGeneral(planetes,m,a,[0.20564, 0.0068, 0.0167, 0.0934, 0.0484, 0.0538, 0.0472, 0.0094],lpr,th,incl)
#Faible variation Neptune (massive + faible excentricité)
R2 = TabSolGeneral(planetes,m,a,[0.20564, 0.0068, 0.0167, 0.0934, 0.0484, 0.05918, 0.0472, 0.0086],lpr,th,incl)
#Faible variation Jupiter (massive + forte excentricité)
R3 = TabSolGeneral(planetes,m,a,[0.20564, 0.00746, 0.0167, 0.0934, 0.0484, 0.0538, 0.0472, 0.0086],lpr,th,incl)
#Faible variation Vénus (léger + faible excentricité)
R4 = TabSolGeneral(planetes,m,a,[0.20564, 0.0068, 0.0167, 0.10274, 0.0484, 0.0538, 0.0472, 0.0086],lpr,th,incl)
#Faible variation Mars (léger + forte excentricité)
R5 = TabSolGeneral(planetes,m,a,[0.20564, 0.0068, 0.0167, 0.0934, 0.0484, 0.0538, 0.0472, 0.0172],lpr,th,incl)
#Forte variation Neptune (léger + forte excentricité)
R6 = TabSolGeneral(planetes,m,a,[0.20564, 0.0068, 0.0167, 0.0934, 0.0484, 0.1076, 0.0472, 0.0086],lpr,th,incl)
#Forte variation Jupiter (léger + forte excentricité)
R7 = TabSolGeneral(planetes,m,a,[0.20564, 0.0136, 0.0167, 0.0934, 0.0484, 0.0538, 0.0472, 0.0086],lpr,th,incl)
#Forte variation Vénus (léger + forte excentricité)
R8 = TabSolGeneral(planetes,m,a,[0.20564, 0.0068, 0.0167, 0.0467, 0.0484, 0.0538, 0.0472, 0.0086],lpr,th,incl)
#Forte variation Mars (léger + forte excentricité) (ici on divie par 2 plutot que multiplier)
R9 = TabSolGeneral(planetes,m,a,[0.20564, 0.0068, 0.0167, 0.0934, 0.0484, 0.0538, 0.0472, 0.0086],lpr,th,incl)
#Faible variation Mars (léger + forte excentricité)
R10 = TabSolGeneral(planetes,m,a,[0.20564, 0.0068, 0.0167, 0.0934, 0.0484, 0.0538, 0.0472, 0.0086],lpr,th,incl)
#Faible variation Mars (léger + forte excentricité)
